...
@interface OfferDetailsViewController () <UIScrollViewDelegate, OferImageBrowserDelegate>
...
- (id)initWithOffer:(Offer *)inOffer
{
    NSParameterAssert(inOffer);
...
- (void)loadView
{
    ...
    [navigationBarView setTitle:_offer.title];
    ...
    [issueDateLabel setText:[dateFormatter stringFromDate:_offer.offerDate]];
    ...
    [detailsLabel setText:_offer.details];
...
- (void)viewDidLoad
{
    ...
    [pageControl setNumberOfPages:_offer.images.count];
    [imagesScrollView setContentSize:CGSizeMake(imagesScrollView.bounds.size.width * _offer.images.count, imagesScrollView.bounds.size.height)];
    
    [_offer.images enumerateObjectsUsingBlock:^(OfferImage *offerImage, NSUInteger idx, BOOL *stop) {
...
- (IBAction)enlargeButtonClicked:(id)sender
{
    ...
    OfferImagesBrowserViewController *imageBrowserViewController = [[OfferImagesBrowserViewController alloc] initWithOfferImages:[_offer.images array]];
